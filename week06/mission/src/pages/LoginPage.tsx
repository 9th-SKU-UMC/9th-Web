...
export default function LoginPage() {
  ...
  const [submitAttempted, setSubmitAttempted] = useState(false); // ★ 추가
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    setSubmitAttempted(true); // ★ 버튼 누른 이후부터 에러 표시
  ...
  return (
    ...
    >
      ...
      {submitAttempted && errors.email && (
        <p className="text-red-500 text-sm">{errors.email}</p> // ★ 제출 후에만 보임
      )}
      ...
      {submitAttempted && errors.password && (
        <p className="text-red-500 text-sm">{errors.password}</p> // ★ 제출 후에만 보임
      )}
